    ...
    <div class="container-fluid">
      ...
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-plus"></i> Sửa tin tức</div>
        <div class="card-body">
		...
		%>
        <form method="post" action="<%=request.getContextPath()%>/admin/news/edit?id_news=<%=objNews.getId_news() %>" enctype="multipart/form-data">
          <div class="form-group">
            <label for="exampleInputPassword1">Tên tin tức</label>
            <input class="form-control" type="text" name="tentin" value="<%=objNews.getName() %>" placeholder="Nhập tên tin" required="required" pattern="[^<&@%#>]{5,500}" requiredmsg="Nhập tên tin theo đúng định dạng chỉ chứa chử cái, khoảng trắng, dấu câu, chữ số tối thiểu 5 ký tự, tối đa 200 ký tự">
            ...
            <textarea rows="6" cols="84"   name="mota" required="required" pattern='[^<&@%#>]{20,1000}' requiredmsg="Nhập mô tả theo đúng định dạng chỉ chứa chử cái, khoảng trắng, dấu câu, chữ số tối thiểu 20 ký tự, tối đa 500 ký tự" ><%=objNews.getDescription() %></textarea>
            <label style="display: block" for="exampleInputPassword1">Chi tiết</label>
             <textarea rows="6" cols="84" id="ck" name="chitiet"><%=objNews.getDetail() %></textarea>
          </div>
          <input class="btn btn-primary btn-block" type="submit" value="Sửa tin">
        </form>
        <%} %>
          </div>
        </div>
