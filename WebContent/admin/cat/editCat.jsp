  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <div class="card mb-3">
        ...
        <form method="post" action="<%=request.getContextPath()%>/admin/cat/edit?id_cat=<%=objCat.getId_cat() %>" >
          <div class="form-group">
            <label for="exampleInputPassword1">Tên danh mục</label>
            <input class="form-control" type="text" name="tendanhmuc" value="<%=objCat.getName()%>" placeholder="Nhập tên danh mục" required="required" required maxlength="50" required minlength="6" pattern="[a-z Á-Ỵá-ỵA-Z0-9]{6,20}" requiredmsg="Nhập trường này theo đúng định dạng chỉ chứa chử cái chữ số và khoảng trắng tối thiểu 5 ký tự, tối đa 50 ký tự" >
          	<label for="exampleInputPassword1">Chọn danh mục cha</label>
          	<select class="form-control" name="parent">
          	
          	<% String selected ="";
          		if(objCat.getId_parent() == 0) selected = "selected='selected'";
          		else selected = "";
          	%>
            	<option value="0" <%=selected %>>Là danh mục gốc</option>
            	...
            	for(Category objRootCat : rootCats) {
            		if(objCat.getId_parent() == objRootCat.getId_cat()) selected = "selected='selected'";
            		else
            			selected = "";
            	%>
            	<option <%=selected %> value="<%=objRootCat.getId_cat() %>"><%=objRootCat.getName() %></option>
            	<% } } %>
            </select>
            ...
            <select class="form-control" name="priority">
            	...
            	for(int i=1;i<=soLuong;i++) {
            		if(i==objCat.getPriority()) selected2 = "selected='selected'";
              		else selected2 = "";
            	%>
            	<option <%=selected2 %> value="<%=i %>"><%=i %></option>
            	<% } } %>
            </select>
          </div>
          <input class="btn btn-primary btn-block" type="submit" value="Sửa danh mục">
        </form>
          </div>
        </div>
      </div>
